...
export default function SeccionExperimento({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <Outer id={embedded ? undefined : 'experimento'} className={embedded ? '' : 'py-16 md:py-24 lg:py-28 px-6 bg-background border-t border-foreground/[0.05]'}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-10">
          ...
          <div className="lg:col-span-3 rounded-2xl border border-foreground/[0.08] bg-foreground/[0.02] p-6 md:p-8">
            ...
            {enviado ? (
              <div className="flex flex-col items-center justify-center py-16 gap-5 text-center">
                ...
                <div>
                  <p className="text-foreground font-bold text-lg mb-1">¡Registro exitoso!</p>
              ...
              <>
                <h3 className="text-base font-bold text-foreground mb-6">Registra tu interés</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-[0.15em] text-foreground/40 mb-2">
                      Contacto <span className="text-foreground/20 font-normal normal-case tracking-normal">(opcional)</span>
                    </label>
                    ...
                    <p className="text-foreground/20 text-[10px] mt-1.5 leading-snug">
                  ...
                  <div ref={instRef} className="relative">
                    ...
                    {showSugerencias && sugerencias.length > 0 && (
                      <ul className="absolute z-30 mt-1 w-full bg-surface-card border border-foreground/[0.1] rounded-xl overflow-auto max-h-52 shadow-2xl">
                        {sugerencias.map(s => (
                          <li
                            key={s}
                            onMouseDown={() => {
                            ...
                            className="px-4 py-2.5 text-sm text-foreground/60 hover:bg-inacap-red/10 hover:text-foreground cursor-pointer transition-colors border-b border-foreground/[0.04] last:border-0"
                          >
                            {s}
                          </li>
                        ))}
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className={`w-full appearance-none bg-foreground/[0.04] border border-foreground/[0.08] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-inacap-red/50 focus:ring-1 focus:ring-inacap-red/20 transition-all cursor-pointer pr-10 ${!intencion ? 'text-foreground/25' : 'text-foreground'}`}
                      >
                        <option value="" disabled className="bg-surface-card text-foreground/40">Selecciona tu intención...</option>
                        {INTENCIONES.map(i => (
                          <option key={i.value} value={i.value} className="bg-surface-card text-foreground">{i.label}</option>
                        ))}
                      </select>
                      <SelectArrow />
                    </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 lg:grid-cols-1 gap-3">
              <div className="rounded-xl border border-foreground/[0.07] bg-foreground/[0.02] p-4 text-center lg:text-left flex flex-col lg:flex-row lg:items-center lg:gap-4">
                <div className="text-2xl md:text-3xl font-black text-foreground leading-none">
                  {visitas > 0 ? visitas.toLocaleString('es-CL') : '—'}
                </div>
                <div className="text-[10px] text-foreground/35 font-medium mt-1 lg:mt-0 leading-tight">Visitas al test card</div>
              </div>
              <div className="rounded-xl border border-foreground/[0.07] bg-foreground/[0.02] p-4 text-center lg:text-left flex flex-col lg:flex-row lg:items-center lg:gap-4">
                <div className="text-2xl md:text-3xl font-black text-inacap-red leading-none">
                  {isLoading ? '—' : exitosos}
                </div>
                <div className="text-[10px] text-foreground/35 font-medium mt-1 lg:mt-0 leading-tight">Respuestas positivas</div>
              </div>
              <div className={`rounded-xl border p-4 text-center lg:text-left flex flex-col lg:flex-row lg:items-center lg:gap-4 transition-colors ${objetivoCumplido ? 'border-green-500/25 bg-green-500/5' : 'border-foreground/[0.07] bg-foreground/[0.02]'}`}>
                ...
                <div className="text-[10px] text-foreground/35 font-medium mt-1 lg:mt-0 leading-tight">Tasa de conversión</div>
              </div>
            </div>
            ...
            <div className={`rounded-xl border p-4 transition-colors ${objetivoCumplido ? 'border-green-500/25 bg-green-500/5' : 'border-yellow-500/15 bg-yellow-500/[0.03]'}`}>
              ...
              <p className="text-foreground/40 text-xs leading-relaxed mb-3">
                ...
                <span className="text-foreground/70 font-semibold">10 o más estudiantes</span>{' '}
            ...
            <div className="rounded-xl border border-foreground/[0.07] bg-foreground/[0.02] p-5 flex-1 min-h-0">
              ...
              {isLoading ? (
                <div className="space-y-3.5">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="pb-3.5 border-b border-foreground/[0.05] last:border-0 last:pb-0 animate-pulse">
                ...
                <p className="text-foreground/20 text-xs">Aún no hay registros. ¡Sé el primero!</p>
              ) : (
                <ul className="space-y-3.5">
                  {ultimos5.map((r, i) => (
                    <li key={r._id ?? i} className="pb-3.5 border-b border-foreground/[0.05] last:border-0 last:pb-0">
                      <div className="flex items-start justify-between gap-2 mb-1">
                        <span className="text-foreground/80 text-sm font-semibold leading-tight">{r.nombre}</span>
                      ...
                      <p className="text-foreground/35 text-[11px] leading-tight mb-0.5 truncate">{r.institucion}</p>
                      <p className="text-foreground/25 text-[11px] mb-1">📍 {r.barrio}</p>
                      {r.motivo && (
                        <p className="text-foreground/20 text-[11px] italic leading-snug line-clamp-2 mb-0.5">"{r.motivo}"</p>
                      )}
